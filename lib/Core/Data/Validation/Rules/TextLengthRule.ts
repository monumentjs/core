...
export default class TextLengthRule implements IValidationRule<any> {
    ...
    constructor(min: number|string = 0, max: number|string = Infinity) {
        ...
        if (this._min >= this._max) {
            throw new ValidationError(`validation.exception.invalidLength`, null, null, this);
        }
    }
    ...
    public validate(model: any, property: string): ValidationResult {
        ...
        if (lessThanMin) {
            errors.push(new ValidationError('validation.error.text.minLength', model, property, this));
        }

        if (greaterThanMax) {
            errors.push(new ValidationError('validation.error.text.maxLength', model, property, this));
        }
