import { Func, Func1, Func2, Func3, ToJSON } from '@monument/core';
...
export interface ReadOnlyCollection<T> extends Sequence<T>, ToJSON<T[]>, ToArray<T> {
  ...
  contains(item: T, comparator: Func2<T, T, boolean>): boolean;
   ...
   */
  except(items: Iterable<T>, comparator: Func2<T, T, boolean>): ReadOnlyCollection<T>;
   ...
   */
  intersect(otherItems: Sequence<T>, comparator: Func2<T, T, boolean>): ReadOnlyCollection<T>;
   ...
   * Produces the set union of two sequences by using a specified IEqualityComparator<TItem>.
   */
  union(otherItems: Sequence<T>, comparator: Func2<T, T, boolean>): ReadOnlyCollection<T>;
