...
export class ArrayList<T> extends ReadOnlyCollectionBase<T> implements List<T>, Cloneable<ArrayList<T>> {
  ...
  addIfAbsent(item: T, comparator: Func2<T, T, boolean> = StrictEquals): boolean {
  ...
  equals(other: ReadOnlyList<T>, comparator: Func2<T, T, boolean> = StrictEquals): boolean {
    if (this === other) {
      return true;
    }
  ...
  indexOf(
    item: T,
    startIndex?: number | Func2<T, T, boolean>,
    count?: number | Func2<T, T, boolean>,
    comparator?: Func2<T, T, boolean>
  ): number {
  ...
  remove(item: T, comparator: Func2<T, T, boolean> = StrictEquals): boolean {
  ...
  removeAll(items: Iterable<T>, comparator: Func2<T, T, boolean> = StrictEquals): boolean {
  ...
  retainAll(items: Iterable<T>, comparator: Func2<T, T, boolean> = StrictEquals): boolean {
    return this.removeBy((ownItem: T): boolean => {
      for (const item of items) {
  ...
  private getLastIndexOfArgs(
    startIndex?: number | Func2<T, T, boolean>,
    count?: number | Func2<T, T, boolean>,
    comparator: Func2<T, T, boolean> = StrictEquals
  ): [number, number, Func2<T, T, boolean>] {
    let _startIndex: number;
    let _count: number;
    let _comparator: Func2<T, T, boolean> = comparator;
