...
import { MutableMultiValueMap } from './MutableMultiValueMap';
...
export class MutableLinkedMultiValueMap<K, V> implements MutableMultiValueMap<K, V> {
  ...
  * keysOf(value: V): Iterable<K> {
  ...
  putMany(key: K, values: Iterable<V>): boolean {
  ...
  filter(predicate: Delegate<[K, V, number], boolean>): ReadOnlyMultiValueMap<K, V> {
    ...
    return new MutableLinkedMultiValueMap({
      * [Symbol.iterator](): Iterator<KeyValuePair<K, V>> {
        let index = 0;
  ...
  map<K2, V2>(
    ...
    keyComparator: Delegate<[K2, K2], boolean> = StrictEquals,
    valueComparator: Delegate<[V2, V2], boolean> = StrictEquals
  ): ReadOnlyMultiValueMap<K2, V2> {
    const self = this;

    return new MutableLinkedMultiValueMap<K2, V2>({
