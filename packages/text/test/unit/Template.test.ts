...
describe('Template', function() {
  ...
  const HELLO_TEMPLATE: Template = new Template(MESSAGE_TEMPLATE);
  const COMICS_VENDOR_TEMPLATE: Template = new Template('I like {comicsVendor} comics');
  const FORMULA_TEMPLATE: Template = new Template(`1 + (2 * [3 + 4]) = {result}`);
