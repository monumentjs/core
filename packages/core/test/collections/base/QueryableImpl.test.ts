import {Queryable, QueryableImpl} from '../../..';
...
describe('QueryableImpl', function () {
    describe('lazy calculations', function () {
        test('concat()', () => {
            const a: number[] = [1, 2, 3];
            const b: number[] = [5, 6, 7];
            const result: Queryable<number> = new QueryableImpl(a).concat(b);
        ...
        test('findAll()', () => {
            ...
            const source = new QueryableImpl(numbers);
        ...
        test('map()', () => {
            const numbers: number[] = [1, 2, 3];
            const source: QueryableImpl<number> = new QueryableImpl(numbers);
            const result: Queryable<number> = source.map((num: number): number => {
                return num ** 2;
            });
    ...
    describe('chaining', function () {
        test('findAll() then map()', function () {
            const numbers: number[] = [1, 2, 3];
            const source: QueryableImpl<number> = new QueryableImpl(numbers);
            const evenNumbers: Queryable<string> = source.findAll(isEven).map((n: number) => {
                return n.toString();
            });
            const oddNumbers: Queryable<string> = source.findAll(isOdd).map((n: number) => {
                return n.toString();
            });
        ...
        test('findAll() then take() then map()', () => {
            const numbers: number[] = [1, 2, 3];
            const source: QueryableImpl<number> = new QueryableImpl(numbers);
            const result: Queryable<string> = source.findAll(isEven).take(3).map((num) => {
                return num.toString();
            });
