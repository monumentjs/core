...
export class LinkedList<T> extends AbstractList<T> implements Cloneable<LinkedList<T>>, Equatable<List<T>> {
    ...
    public equals(
        ...
        for (let otherItem of otherList) {
            if (node) {
                if (!this.checkItemsEquality(otherItem, node.nodeValue, comparator)) {
                    return false;
                }
    ...
    public indexOf(
        ...
        return LinkedList.INDEX_NOT_FOUND;
    }
    ...
    public lastIndexOf(
        ...
        if (startIndex == null && count == null) {
            for (let index = this.length - 1, node = this._lastNode; index >= 0 && node != null; index--, node = node.previousNode) {
                ...
                if (this.checkItemsEquality(searchItem, currentItem, comparator)) {
                    return index;
                }
            }
        }
    ...
    private findNode(value: T, comparator?: EqualityComparator<T>): LinkedListNode<T> | undefined {
        ...
        this.forEachNode((node: LinkedListNode<T>): boolean | void => {
            if (this.checkItemsEquality(node.nodeValue, value, comparator)) {
                result = node;
