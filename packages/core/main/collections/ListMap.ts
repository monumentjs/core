...
export class ListMap<K, V> extends AbstractMap<K, V> {
    ...
    public put(key: K, value: V): V | undefined {
        ...
        for (const pair of this._mapping) {
            if (this.checkItemsEquality(pair.key, key, this.keyComparator)) {
    ...
    public putAll(values: Iterable<KeyValuePair<K, V>>): boolean {
        ...
        for (const newPair of values) {
            ...
            for (const pair of this._mapping) {
                if (this.checkItemsEquality(pair.key, newPair.key, this.keyComparator)) {
                    this._mapping.setAt(index, newPair);
    ...
    public removeIf(key: K, value: V): boolean {
        ...
        for (const pair of this._mapping) {
            if (this.checkItemsEquality(key, pair.key, this.keyComparator) &&
                this.checkItemsEquality(value, pair.value, this.valueComparator)) {
    ...
    public replaceIf(key: K, oldValue: V, newValue: V): boolean {
        ...
        for (const pair of this._mapping) {
            if (this.checkItemsEquality(key, pair.key, this.keyComparator) && this.checkItemsEquality(oldValue, pair.value, this.valueComparator)) {
                this._mapping.setAt(index, new KeyValuePair(key, newValue));
    ...
    public containsValue(value: V): boolean {
        for (const pair of this._mapping) {
            if (this.checkItemsEquality(pair.value, value, this.valueComparator)) {
                return true;
            }
        }
    ...
    public containsEntry(key: K, value: V): boolean {
        for (const pair of this._mapping) {
            if (this.checkItemsEquality(pair.key, key, this.keyComparator) &&
                this.checkItemsEquality(pair.value, value, this.valueComparator)) {
                return true;
            }
        }
